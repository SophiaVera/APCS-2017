 ...
 * <p>
 * The modification to this class incorporate the card and deck objects into poker, with its
 *  different terms for hands and its calculations for getting them.
 *
 * @author Allen B. Downey - original author
 * @author Sophia Vera - modifications
 **/
public class CardSoln3 {
    ...
    public static void main(String[] args) {
		...
		PokerHand pokerHand = new PokerHand(5);
		
		//makes 6 poker hands and prints them
    	for (int i = 0; i < 6; i++) {
    		System.out.println("-------------");
    		pokerHand = deck.deal();
    		for (int j = 0; j < 5; j++) {
    			pokerHand.cards[j].print();
    		}
    		System.out.println("-------------");
    	}
    	
    	//deals a lot of poker cards, and checks probability
		deck.dealThousands();
...
class Deck {
    ...
    public boolean checkInDeck(Card card, Deck deck){
    	for (int i = 0; i < deck.cards.length; i++) {
    		if(deck.cards[i] == null){
    			continue;
    		}
    		else if(card.toString().equals(deck.cards[i].toString())){
				return false;
			}
		}
    	return true;
    ...
    /**
     * This method creates and returns a poker hand composed of 5 random cards.  Instead of randomly picking
     * cards from the deck, it randomly shuffles them and takes the first card, which is effectively the same thing.
     * It also sorts the poker hand to make it more readable.
     * 
     * @return pokerHand - a group of 5 cards 
     * @author Sophia Vera
     */
    public PokerHand deal(){
    	PokerHand pokerHand = new PokerHand(5);
    	for (int i = 0; i < 5; i++) {
    		while(!checkInDeck(this.cards[0], pokerHand)){
        		this.shuffle();
    		}
    		Card result = this.cards[0];
    		pokerHand.cards[i] = result;
    		this.shuffle();
    	}
    	pokerHand.sort();
    	return pokerHand;
    ...
    /**
     * The dealThousands() method creates an enormous list of poker hands and then calculates the percent of them 
     * that are either flushes or have groups of three.  It prints these values as a percent.
     */
    public void dealThousands(){
    	PokerHand[] hands = new PokerHand[100000];
    	...
    	for (int i = 0; i < hands.length; i++) {
			...
			else if (hands[i].hasThreeKind()){ //else if because all flushes have three of the same kind
				threeCounter += 1;
			}
		}
    	
    	System.out.println("Percent flushes: " + (flushCounter/100000)*100 + " percent");
    	System.out.println("Percent three of a kind: " + (threeCounter/100000)*100 + " percent");
    }

}

/**
 * PokerHand is a class extended from Deck which has the attributes of a set of cards from the game of poker.  It
 * can calculate whether a hand meets the requirements for a special name in the game.
 * @author Sophia Vera
 */
class PokerHand extends Deck{
	 public PokerHand(int n) {// n can be 5 or 7, or the desired length of the hand
	        this.cards = new Card[n];
	 }
	 
	 /**
	  * In poker, a flush is defined as being a hand where everything is of the same suit.  
	  * 
	  * @return whether the hand has a flush or not
	  */
	 ...
	 /**
	  * In poker, having three of the same kind is defined as having three cards of the same rank.
	  * @return whether the hand has a group of three or not
	  */
	 public boolean hasThreeKind(){
		 sort();
		 int currentChecking = cards[0].rank;
		 int counter = 0;
		 for (int i = 0; i < cards.length; i++) {
			 currentChecking = cards[i].rank;
			 for (int j = 0; j < cards.length; j++) {
				 if(cards[j].rank == currentChecking){
					 counter += 1;
				 }
				 if(counter == 3){
					 return true;
